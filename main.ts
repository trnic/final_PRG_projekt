import { rawTrainings, ITrainingData, TrainingComparison, TrainingComputedValues, TrainingDifficultyLabel, TrainingRecord, TrainingValidationIssue, TrainingValidationResult, SprintTrainingData, EnduranceTrainingData, ShootingTrainingData, SkillsTrainingData, TrainingType, TrainingTrendAnalysis } from './data';
...
function formatDifficulty(score: number): TrainingDifficultyLabel {
    if (score < 35) return 'nízká';
    if (score < 70) return 'střední';
    return 'vysoká';
}

function formatTrainingDate(performedAt: string): string {
    return new Date(performedAt).toLocaleString('cs-CZ');
}

function validateTrainingData(data: ITrainingData): TrainingValidationResult {
...
function formatValidationErrors(issues: TrainingValidationIssue[]): string {
    return issues.map(issue => issue.message).join(' ');
}

function createValidationErrorMessage(index: number, data: ITrainingData, issues: TrainingValidationIssue[]): string {
    return `Záznam č. ${index + 1} (${data.type}) je neplatný: ${formatValidationErrors(issues)}`;
}

function readFormInputsForTraining(element: HTMLDetailsElement): number[] {
    const inputs = element.querySelectorAll<HTMLInputElement>('.training-fields input[type="number"]');
    return Array.from(inputs).map(input => parseFloat(input.value) || 0);
}

function buildTrainingDataFromForm(trainingType: string, formValues: number[]): ITrainingData | null {
    const type = trainingType as TrainingType;
    ...
    switch (type) {
        case 'sprint':
            ...
            } as SprintTrainingData;
        case 'endurance':
            return {
                ...baseData,
                averageHeartRate: formValues[2],
            } as EnduranceTrainingData;
        case 'shooting':
            return {
                ...baseData,
                shots: formValues[2],
                accuracy: formValues[3],
            } as ShootingTrainingData;
        case 'skills':
            return {
                ...baseData,
                technique: formValues[2],
                ballControl: formValues[3],
            } as SkillsTrainingData;
        default:
            return null;
    }
}

function analyzeUserTraining(formData: ITrainingData, previousRecord: TrainingRecord | null, previousScore: number | null, allAnalyses: TrainingAnalysis[]): TrainingAnalysis | null {
...
function createFormSubmitHandler(detailsElement: HTMLDetailsElement, analysisHistory: TrainingAnalysis[], output: HTMLDivElement | null) {
    return function (event: Event) {
        ...
        try {
            ...
            const previousRecord: TrainingRecord | null = previousAnalysis
                ? ({
                      ...
                      ...(previousAnalysis.type === 'sprint' && { distance: (previousAnalysis as any).distance }),
                      ...(previousAnalysis.type === 'endurance' && { averageHeartRate: (previousAnalysis as any).averageHeartRate }),
                      ...(previousAnalysis.type === 'shooting' && { shots: (previousAnalysis as any).shots, accuracy: (previousAnalysis as any).accuracy }),
                      ...(previousAnalysis.type === 'skills' && { technique: (previousAnalysis as any).technique, ballControl: (previousAnalysis as any).ballControl }),
                  } as TrainingRecord)
                : null;
...
function initializeFormHandlers(analysisHistory: TrainingAnalysis[], output: HTMLDivElement | null): void {
    const detailsElements = document.querySelectorAll<HTMLDetailsElement>('.training-item[data-type]');

    detailsElements.forEach(details => {
        const button = details.querySelector<HTMLButtonElement>('.btn-submit');
        if (button) {
...
type TrainingAnalysis = TrainingRecord & TrainingComputedValues & {
    index: number;
    summary: string;
    difficultySummary: string;
    comparison: TrainingComparison;
    trend: TrainingTrendAnalysis;
};
...
const TRAINING_FIELD_MAPPING: FormFieldMapping = {
...
abstract class Training {
    ...
    abstract calculateDifficultyScore(): number;
    ...
    public getDifficultySummary(): string {
...
class SprintTraining extends Training {
    ...
    calculateDifficultyScore(): number {
...
function createTrainingFromData(data: ITrainingData): Training {
...
function buildTrainingRecord(data: ITrainingData, index: number): TrainingRecord {
    return {
...
function calculateTrainingValues(training: Training): TrainingComputedValues {
...
function compareTrainingDifficulty(currentRecord: TrainingRecord, currentScore: number, previousRecord: TrainingRecord | null, previousScore: number | null): TrainingComparison {
...
function calculateTrainingTrend(analyses: TrainingAnalysis[], currentIndex: number): TrainingTrendAnalysis {
    ...
    let trendingUp: boolean | null = null;
    let trendSummary = '';
...
function analyzeTraining(training: Training, sourceRecord: TrainingRecord, index: number, previousRecord: TrainingRecord | null, previousScore: number | null, allAnalyses?: TrainingAnalysis[]): TrainingAnalysis {
...
function analyzeTrainings(trainingData: ITrainingData[]): TrainingAnalysis[] {
    const analyses: TrainingAnalysis[] = [];
...
function logTrainingAnalysis(analyses: TrainingAnalysis[]): void {
...
function renderTrainingAnalysis(analyses: TrainingAnalysis[], output: HTMLDivElement | null): void {
    if (!output) {
        return;
    }
...
function renderValidationError(message: string, output: HTMLDivElement | null): void {
    if (!output) {
        return;
    }
...
function renderTrainingHistory(analyses: TrainingAnalysis[], output: HTMLDivElement | null): void {
